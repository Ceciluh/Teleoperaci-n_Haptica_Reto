...
BD_IMP = 70.0    # amortiguamiento virtual [N·s/m]
...
class PegHoleContact:
    ...
    def compute_contact_force(self, x_ef):
        ...
        if dist < HOLE_RADIUS * 3.0: 
            self.phase = max(self.phase, self.INSERTION)
            self.time_inside += DT

            if dist > HOLE_RADIUS:   
                penetration = dist - HOLE_RADIUS
                F_contact   = -F_CONTACT_K * penetration * (delta / dist)
                in_contact  = True

            if self.time_inside >= 5.0:
...
def impedance_control(q, dq, x_des, dx_des, q_des_prev,
    ...
    # Fuerza de obstcolas
    F_obs = obstacle_force(x_cur, J @ dq)
...
class SlaveRobot:
    ...
    def step(self):
        ...
        self.hist_tau[i] = tau
#--------------------------------------
        J = jacobian_3r(self.q)
        self.hist_Fc[i] = F_contact + obstacle_force(fk_3r(self.q), J @ self.dq)
#--------------------------------------
...
def _save_slave_data(robot):
    import os
    os.makedirs("data", exist_ok=True)
    os.makedirs("figures", exist_ok=True)
    # número de ensayo automático
    i = 1
    while os.path.exists(f"data/slave_t_{i}.csv"):
        i += 1
    n   = min(robot.idx, 500)
    i0  = robot.idx % 500
    ...
    np.savetxt(f"data/slave_t_{i}.csv",   t,   delimiter=",")
    np.savetxt(f"data/slave_tau_{i}.csv", tau, delimiter=",", header="tau1,tau2,tau3")
    np.savetxt(f"data/slave_Fc_{i}.csv",  Fc,  delimiter=",", header="Fx,Fy")
    np.savetxt(f"data/slave_x_{i}.csv",   x,   delimiter=",", header="x,y")
    np.savetxt(f"data/slave_ex_{i}.csv",  ex,  delimiter=",", header="ex,ey")
    print(f"[slave] ensayo {i} guardado en data/")
    fig2, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig2.suptitle(f"Slave — Ensayo {i}", color='black', fontsize=13)
    axes[0,0].set_title("Torques [Nm]")
    for j in range(3):
        axes[0,0].plot(t, tau[:,j], label=f"tau{j+1}")
    axes[0,0].legend(); axes[0,0].grid(True)
    axes[0,1].set_title("Fuerzas de contacto [N]")
    axes[0,1].plot(t, Fc[:,0], label="Fx")
    axes[0,1].plot(t, Fc[:,1], label="Fy")
    axes[0,1].legend(); axes[0,1].grid(True)
    axes[1,0].set_title("Error cartesiano [mm]")
    axes[1,0].plot(t, np.abs(ex[:,0])*1000, label="|ex|")
    axes[1,0].plot(t, np.abs(ex[:,1])*1000, label="|ey|")
    ...
    axes[1,1].grid(True)
    plt.tight_layout()
    fig2.savefig(f"figures/slave_plots_{i}.png", dpi=150)
    plt.close(fig2)
    print(f"[slave] figura guardada en figures/slave_plots_{i}.png")
...
def main(args=None):
    parser = argparse.ArgumentParser(description="Robot Esclavo xArm6")
    parser.add_argument("--master-ip", default="192.168.1.195",
                        help="IP del PC maestro")
    args = parser.parse_args()
    _main(args.master_ip)
