...
from obstacles import OBSTACLES, K_OBS, obstacle_force
...
class MasterRobot:
    ...
    def __init__(self, slave_ip="127.0.0.1"):
        ...
        self.v_step = 0.60  # [m/s] por pulsación de tecla
        ...
        self._ros_node = _MasterServoNode()
        self._ros_thread = threading.Thread(
        target=_ros_spin, args=(self._ros_node,), daemon=True)
    ...
    def step(self):
        ...
        #-------------------------------------------------
        x_des = x_cur + self.v_cart * DT + (obstacle_force(x_cur, self.v_cart) / 800.0) * DT
        ...
        tau, e, de = computed_torque(
            ...
            F_ext = 0.3 * self.net.Fe + obstacle_force(fk_3r(self.q), self.v_cart)
        )
         #-------------------------------------------------
        ...
        #obstacle_force(fk_3r(self.q), self.v_cart)
        ...
        self.hist_tau[i] = tau
        #---------------------------------------------------
        self.hist_Fe[i] = self.net.Fe + obstacle_force(fk_3r(self.q), self.v_cart)
        #---------------------------------------------------
        self.hist_x[i]   = x_ef
        self.idx += 1
        self.t   += DT
...
# ====================================================
    for obs in OBSTACLES:
        ...
        rect = plt.Rectangle((x_min, y_min), x_max-x_min, y_max-y_min, color='red', alpha=0.4)
...
class _MasterServoNode(Node):
    def __init__(self):
        if not rclpy.ok():
            rclpy.init()
        super().__init__("master_servo_node")
        self._pub = self.create_publisher(
            TwistStamped, "/servo_server/delta_twist_cmds", 10)

    def publish_vcart(self, v):
        """Publica v[0]=vx, v[1]=vy como TwistStamped en link_base."""
        msg = TwistStamped()
...
def _save_data(robot):
    import os
    os.makedirs("data", exist_ok=True)
    os.makedirs("figures", exist_ok=True)
    i = 1
    while os.path.exists(f"data/master_t_{i}.csv"):
        i += 1
    n   = min(robot.idx, 500)
    i0  = robot.idx % 500
    ...
    Fe  = robot.hist_Fe[idx]
    x   = robot.hist_x[idx]
    np.savetxt(f"data/master_t_{i}.csv",   t,   delimiter=",")
    np.savetxt(f"data/master_q_{i}.csv",   q,   delimiter=",", header="q1,q2,q3")
    np.savetxt(f"data/master_tau_{i}.csv", tau, delimiter=",", header="tau1,tau2,tau3")
    np.savetxt(f"data/master_Fe_{i}.csv",  Fe,  delimiter=",", header="Fx,Fy")
    np.savetxt(f"data/master_x_{i}.csv",   x,   delimiter=",", header="x,y")
    print(f"[master] ensayo {i} guardado en data/")
    fig2, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig2.suptitle(f"Master — Ensayo {i}", color='black', fontsize=13)
    axes[0,0].set_title("Torques [Nm]")
    for j in range(3):
        axes[0,0].plot(t, tau[:,j], label=f"tau{j+1}")
    axes[0,0].legend(); axes[0,0].grid(True)
    axes[0,1].set_title("Fuerzas reflejadas [N]")
    axes[0,1].plot(t, Fe[:,0], label="Fx")
    axes[0,1].plot(t, Fe[:,1], label="Fy")
    axes[0,1].legend(); axes[0,1].grid(True)
    axes[1,0].set_title("Angulos articulares [rad]")
    for j in range(3):
        axes[1,0].plot(t, q[:,j], label=f"q{j+1}")
    axes[1,0].legend(); axes[1,0].grid(True)
    axes[1,1].set_title("Trayectoria EF [m]")
    axes[1,1].plot(x[:,0], x[:,1])
    axes[1,1].set_xlabel("x"); axes[1,1].set_ylabel("y")
    axes[1,1].grid(True)
    plt.tight_layout()
    fig2.savefig(f"figures/master_plots_{i}.png", dpi=150)
    plt.close(fig2)
    print(f"[master] figura guardada en figures/master_plots_{i}.png")
...
def _main(slave_ip):
    ...
    def on_key_press(event):
        ...
        if   event.key == 'w': robot.v_cart = np.array([0,  v])
        elif event.key == 'x': robot.v_cart = np.array([0, -v])
            ...
            plt.close()
            return
        robot._ros_node.publish_vcart(robot.v_cart)  
